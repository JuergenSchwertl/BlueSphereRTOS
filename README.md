# Azure RTOS based C++ sample for Azure Sphere

## Introduction

This repo contains an exemplary C++ real-time capable sample application running under Azure RTOS on the MT3620 M-cores.
It also has step by step instructions on how to get setup for your own app journey.

Key learnings in this module:
* Development toolchains
* Debugging and debug logging considerations with real-time capable applications
* Mediatek M4 Driver package
* Azure RTOS for the MT3620
* Application startup and enabling C++ for real-time capable applications
* 
...
## Development toolchain for real-time capable applications
What we need is a toolchain that allows cross compilation to ARM and a stdlib iimplementation that does not require a target operating system:
 **ARM-NONE-EABI**.
...
*C:\Program Files (x86)\Microsoft Visual Studio\2019\Community\Linux\gcc_arm* 
> I'm running *Visual Studio Community Edition* and *Visual Studio Code* on the same machine and therefore have settings.json in 
> VSC refer to the VS-Toolchain for the "AzureSphere.ArmGnuPath". For other Visual Studio editions, the path slightly varies as mentioned above). 
...
### Debugging and debug logging considerations
...
for a number of CPU cycles. It's therefore a bit tricky to debug the application start unless you use a little workaround: 
add a loop waiting for a local variable to change and set a break point inside the loop. The app will start and spinning cycles 
in the loop until you *break* into the debugger and change the variable (e.g. press Pause to stop execution and set `nWait = 0` in the debugger to continue). 
```C
...
Since we do not have any operating system available and thus also no _STDOUT_ or *Log_Debug()* on the M4 cores we want to use a UART to output our debugging messages.
...
You could elect to add individual hardware related sources (i.e. os_hal_gpio.c ) into your project directly or build them all together as a library and let the linker decide
...
- To initializethe UART we call the OS_HAL function [mtk_os_hal_uart_ctlr_init() in os_hal_uart.c](https://github.com/MediaTek-Labs/mt3620_m4_software/blob/master/MT3620_M4_Sample_Code/OS_HAL/src/os_hal_uart.c#L131)
...
Let's now have a look on the detailed startup sequence and what it takes to also enable C++ on the M4 core:

## Real-time capable application startup
...
Once that startup is done, we'll continue in [`main()`](./BlueSphereRTOS/src/main.cpp#113) and start Azure RTOS.
